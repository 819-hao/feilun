 ...
 */
@Service
@Extension(bizId = BizCode.TO_B_SALE_RETURN, useCase = UseCase.CANCEL)
public class SaleReturnOrderCancelExtToB implements CancelExtPtI<SaleReturnOrderCancelRequest, SaleReturnOrderCancelResult> {
    ...
    public List<StockLifeCycleResult> lifeCycle(SaleReturnOrderCancelRequest request, SaleReturnOrderCancelResult result) {
        return result.getLine().stream().map(saleReturnOrderLineVO -> StockLifeCycleResult.builder()
                ...
                .operationDesc(String.format(OperationDescConst.SALE_RETURN_CANCEL, OperationDescConst.SALE_RETURN_CREATE_TH))
