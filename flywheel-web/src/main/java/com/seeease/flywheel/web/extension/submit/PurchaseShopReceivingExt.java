 ...
 */
@Service
@Extension(bizId = BizCode.PURCHASE, useCase = UseCase.SHOP_RECEIVING)
public class PurchaseShopReceivingExt implements SubmitExtPtI<PurchaseExpressNumberUploadRequest, PurchaseExpressNumberUploadListResult> {
    ...
    public PurchaseExpressNumberUploadListResult submit(SubmitCmd<PurchaseExpressNumberUploadRequest> cmd) {
        ...
        return purchaseFacade.shopReceiving(cmd.getRequest());
    }
    ...
    public List<StockLifeCycleResult> lifeCycle(PurchaseExpressNumberUploadRequest request, PurchaseExpressNumberUploadListResult result) {
        return Arrays.asList(StockLifeCycleResult.builder()
                ...
                .operationDesc(OperationDescConst.SHOP_RECEIVING)
                .build());
    }
}
