...
@Slf4j
@Service
@Extension(bizId = BizCode.IMPORT, useCase = UseCase.PRICING_PASS)
public class PricingBatchImportPassExt implements ImportExtPtl<PricingStockQueryImportRequest, PricingStockQueryImportResult> {
...
//        taskBatchQueryRequest.setTaskDefinitionKeys(Lists.newArrayList(TaskDefinitionKeyEnum.WAIT_CHECK.getKey()));
...
//                .forEach(t -> {
//
//                    PricingCompletedRequest  request = PricingCompletedRequest.builder()
//                            .id(t.getId())
...
//
//
//                    SubmitCmd<PricingCompletedRequest> pricingFinishCmd = new SubmitCmd<>();
//                    pricingFinishCmd.setBizCode(BizCode.PRICING);
//                    pricingFinishCmd.setUseCase(UseCase.REPAIR_COMPLETED);
//                    pricingFinishCmd.setRequest(request);
//
//
//                    TaskDTO task = taskMap.get(t.getSerialNo());
//                    UserTaskDto userTask = UserTaskDto.builder()
...
//                            .build();
//                    pricingFinishCmd.setTaskList(Collections.singletonList(userTask));
//
