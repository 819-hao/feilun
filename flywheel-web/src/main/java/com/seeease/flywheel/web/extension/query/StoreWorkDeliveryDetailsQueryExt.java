 ...
 */
@Service
@Extension(bizId = BizCode.STORAGE, useCase = UseCase.LOGISTICS_DELIVERY_DETAILS)
public class StoreWorkDeliveryDetailsQueryExt implements QueryExtPtI<StoreWorkDeliveryDetailRequest> {
    ...
    public QueryResult query(QueryCmd<StoreWorkDeliveryDetailRequest> cmd) {
        ...
        List<StoreWorkDetailResult> result = storeWorkQueryFacade.deliveryDetail(cmd.getRequest());
        List<QuerySingleResult> collect = result
                .stream().map(t -> QuerySingleResult.builder().result(t)
    ...
    public List<TaskDefinitionKeyEnum> needQueryTaskKeys(QueryCmd<StoreWorkDeliveryDetailRequest> cmd) {
        return Lists.newArrayList(TaskDefinitionKeyEnum.LOGISTICS_DELIVERY);
    }
