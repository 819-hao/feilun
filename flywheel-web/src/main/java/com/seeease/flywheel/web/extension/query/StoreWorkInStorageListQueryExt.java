 ...
 */
@Service
@Extension(bizId = BizCode.STORAGE, useCase = UseCase.IN_STORAGE_LIST)
public class StoreWorkInStorageListQueryExt implements QueryExtPtI<StoreWorkListRequest> {
    ...
    public QueryResult query(QueryCmd<StoreWorkListRequest> cmd) {
        ...
        PageResult<StoreWorkListResult> result = storeWorkQueryFacade.listInStorage(request);
    ...
    public List<TaskDefinitionKeyEnum> needQueryTaskKeys(QueryCmd<StoreWorkListRequest> cmd) {
        return Lists.newArrayList(TaskDefinitionKeyEnum.IN_STORAGE);
    }
