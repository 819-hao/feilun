 ...
 */
@Service
@Extension(bizId = BizCode.TO_C_SALE_RETURN, useCase = UseCase.PROCESS_CREATE)
public class SaleReturnCreateExtToC implements CreateExtPtI<SaleReturnOrderCreateRequest, SaleReturnOrderCreateResult> {
    ...
    public List<ProcessInstanceStartDto> start(SaleReturnOrderCreateRequest request, SaleReturnOrderCreateResult result) {
        return result.getList().stream().map(t -> {
            ...
            return ProcessInstanceStartDto.builder().serialNo(t.getSerialNo()).process(ProcessDefinitionKeyEnum.TO_C_SALE_RETURN).variables(workflowVar).build();
        }).collect(Collectors.toList());
    }

    @Override
    public List<StockLifeCycleResult> lifeCycle(SaleReturnOrderCreateRequest request, SaleReturnOrderCreateResult result) {
        return result.getList().stream().map(t -> t.getStockIdList().stream().map(id -> StockLifeCycleResult.builder().originSerialNo(t.getSerialNo()).stockId(id).operationDesc(String.format(OperationDescConst.SALE_RETURN_CREATE, OperationDescConst.SALE_RETURN_CREATE_GR)).build()).collect(Collectors.toList())).flatMap(Collection::stream).collect(Collectors.toList());
    }
