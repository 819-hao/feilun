 ...
 */
@Service
@Extension(bizId = BizCode.TO_C_SALE_RETURN, useCase = UseCase.UPLOAD_EXPRESS_NUMBER)
public class SaleReturnOrderExpressNumberUploadExtToC implements SubmitExtPtI<SaleReturnOrderExpressNumberUploadRequest, SaleReturnOrderExpressNumberUploadResult> {
    ...
    public SaleReturnOrderExpressNumberUploadResult submit(SubmitCmd<SaleReturnOrderExpressNumberUploadRequest> cmd) {
        if (cmd.getRequest().getTiktokSaleReturnShopId() != null){
            cmd.getRequest().setStoreId(cmd.getRequest().getTiktokSaleReturnShopId());
        }else {
    ...
    public List<StockLifeCycleResult> lifeCycle(SaleReturnOrderExpressNumberUploadRequest request, SaleReturnOrderExpressNumberUploadResult result) {
        return result.getStoreWorkList().stream().map(storeWorkCreateResult -> StockLifeCycleResult.builder()
                .stockId(storeWorkCreateResult.getStockId())
                .originSerialNo(result.getSerialNo())
                .operationDesc(String.format(OperationDescConst.SALE_UPLOAD_EXPRESS, OperationDescConst.SALE_RETURN_CREATE_GR)).build()).collect(Collectors.toList());
    }
}
