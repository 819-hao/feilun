...
@Slf4j
@Component
@RocketMQMessageListener(topic = "feilunInvoiceMaycurCreateRes", consumerGroup = "feilunInvoiceMaycurGroup_CreateRes", selectorExpression = "*",
        messageModel = MessageModel.CLUSTERING, consumeMode = ConsumeMode.CONCURRENTLY, nameServer = FlywheelConstant.MQ_NAME_SERVE)
public class FeilunInvoiceMaycurCreateResListener implements RocketMQListener<JSONObject> {
    ...
    public void onMessage(JSONObject message) {
        log.info("onMessage function of feilunInvoiceMaycurCreateRes start and request = {}", message);
        ...
        if (!flag) {
            facade.maycurInvoice(FinancialInvoiceMaycurRequest.builder()
                    ...
                    .useScenario(FinancialInvoiceMaycurRequest.UseScenario.CREATE)
                    .build());
        }
