 ...
 */
@Component
public class QtWarningStrategy extends QtDecisionStrategy {
    ...
    public QualityTestingStateEnum getState() {
        return QualityTestingStateEnum.ANOMALY;
    }
    ...
    public QualityTestingDecisionListResult save(QualityTestingDecisionRequest request) {
        ...
        return transactionalUtil.transactional(() -> {
            ...
            billStoreWorkPreService.qtPassed(qualityTestingDetailsResult.getWorkId(), WhetherEnum.YES);
            ...
            billQualityTesting.setExceptionReason(request.getExceptionReason());
            billQualityTesting.setExceptionReasonId(request.getExceptionReasonId());

            Stock stock = new Stock();
            stock.setId(qualityTestingDetailsResult.getStockId());
            stock.setUnusualDesc(request.getExceptionReason());
            ...
            //日志
            logQualityTestingOpt.setQtState(QualityTestingStateEnum.TransitionEnum.RECEIVE_ANOMALY_DELIVERY.getToState());
            logQualityTestingOpt.setQtConclusion(QualityTestingConclusionEnum.ANOMALY);
            logQualityTestingOpt.setExceptionReason(request.getExceptionReason());
            logQualityTestingOpt.setExceptionReasonId(request.getExceptionReasonId());

            super.optAndDecisionSave(billQualityTesting, logQualityTestingOpt);

            billHandlerEventPublisher.publishEvent(new QtDecisionEvent(qualityTestingDetailsResult.getStockId(), QualityTestingStateEnum.ANOMALY, qualityTestingDetailsResult.getOriginSerialNo()
...
//        }
        ...
        Assert.isTrue(!ANOMALY_EXCLUDE_LIST.contains(billQualityTestingMapper.selectOne(Wrappers.<BillQualityTesting>lambdaQuery().eq(BillQualityTesting::getId, request.getQualityTestingId())).getQtSource()), "质检不能判断异常");
    }
}
