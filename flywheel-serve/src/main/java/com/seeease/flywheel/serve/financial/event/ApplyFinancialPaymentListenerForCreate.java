...
@Slf4j
@Component
public class ApplyFinancialPaymentListenerForCreate implements BillHandlerEventListener<ApplyFinancialPaymentCreateEvent> {
    ...
    @Override
    public void onApplicationEvent(ApplyFinancialPaymentCreateEvent event) {
        log.info("onApplicationEvent function of ApplyFinancialPaymentListenerForCreate start and event = {}", JSON.toJSONString(event));
        ...
        if (TYPE_ENUMS.contains(event.getTypePayment())) {
            ...
            customerBalanceService.customerBalanceCmd(event.getCustomerId(), event.getContactId(), event.getClinchPrice(),
                    ...
                    CustomerBalanceCmdTypeEnum.MINUS.getValue(), event.getUserId(), event.getOriginSerialNo());
        }
